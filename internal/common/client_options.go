...
func (o ClientOptions) requestLogger(req *http.Request) (*http.Request, error) {
	...
	if dump, err := httputil.DumpRequestOut(newReq, true); err == nil {
		log.Printf(`[DEBUG] ============================ Begin GroupManager Request ============================
Request ID: %s

%s
============================= End GroupManager Request =============================
`, requestId, dump)
	} else {
		// fallback to basic message
		log.Printf("[DEBUG] GroupManager Request %s: %s %s\n", requestId, newReq.Method, newReq.URL)
	}
...
func (o ClientOptions) responseLogger(req *http.Request, resp *http.Response) (*http.Response, error) {
	...
	if resp != nil {
		if dump, err2 := httputil.DumpResponse(resp, true); err2 == nil {
			log.Printf(`[DEBUG] ============================ Begin GroupManager Response ===========================
%s %s
Request ID: %s

%s
============================= End GroupManager Response ============================
`, req.Method, req.URL, requestId, dump)
		} else {
			log.Printf("[DEBUG] GroupManager Response: %s for %s (%s %s)\n", resp.Status, requestId, req.Method, req.URL)
		}
	} else {
		log.Printf("[DEBUG] GroupManager Request for %s (%s %s) completed with no response", requestId, req.Method, req.URL)
	}
...
func (o ClientOptions) userAgent(sdkUserAgent string) (userAgent string) {
	...
	providerUserAgent := fmt.Sprintf("%s terraform-provider-groupmanager/%s", tfUserAgent, version.ProviderVersion)
